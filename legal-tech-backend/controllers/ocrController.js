...
const normalizeNepali = (s = "") =>
  s.normalize("NFC").replace(/\u200B|\u200C|\u200D|\u00AD/g, "").replace(/[“”]/g, '"').replace(/[‘’]/g, "'").trim();
...
const typeToSection = (t = "") => ({
  POA: "poa", Petition: "petition", Reply: "reply", Evidence: "evidence",
  "Interim Order": "order", Testimonial: "other", "District Judgment": "judgment",
  "High Court Appeal": "appeal", "High Court Judgment": "judgment",
  "Supreme Court Appeal": "appeal", "Supreme Court Judgment": "judgment",
}[t] || "other");
...
async function assertCaseAccess(req, caseId) {
  ...
  if (!ok) {
    ...
    err.statusCode = existsInOrg ? 403 : 404; throw err;
  }
}
const isWorker = (req) => !!req.headers["x-ocr-worker-key"] && req.headers["x-ocr-worker-key"] === process.env.OCR_WORKER_KEY;
...
export const startOcrJob = async (req, res) => {
  try {
    ...
    job.status = "running"; job.engine = engine; job.startedAt = new Date();
    await job.save();

    await Document.updateOne({ _id: job.documentId }, { $set: { "ocr.status": "running" } });
...
export const saveOcrResult = async (req, res) => {
  try {
    ...
    const doc = await Document.findById(documentId);
    if (!doc) return res.status(404).json({ message: "Document not found" });
    if (!isWorker(req)) await assertCaseAccess(req, doc.caseId);

    const { fullText, avgConfidence, perPage = [], autoSections = [], entities = [], searchHints = [], extraction = {}, metrics } = req.body || {};
    if (!fullText || typeof fullText !== "string") return res.status(400).json({ message: "fullText (string) is required" });

    const orgId = doc.orgId || req.orgId;
    ...
    const pages = perPage.map((p) => ({ page: p.page, confidence: typeof p.confidence === "number" ? p.confidence : undefined, textLen: typeof p.textLen === "number" ? p.textLen : (p.text ? p.text.length : undefined), textDensity: p.textDensity }));
    ...
    const upsert = {
      ...
      normalization: { version: "v1", needsReview: typeof avgConfidence === "number" ? avgConfidence < 0.85 : false, scriptRatio: { devanagari: 0, latin: 0 }, garbageRate: metrics?.garbageRate ?? 0 },
      pages,
      autoSections,
      extraction,
      textHash,
    ...
    const docText = await DocumentText.findOneAndUpdate(
      { documentId: doc._id, orgId },
      upsert,
      { upsert: true, new: true, setDefaultsOnInsert: true }
    );

    doc.ocr = doc.ocr || {};
    if (typeof avgConfidence === "number") { doc.ocr.avgConfidence = avgConfidence; doc.ocr.needsReview = avgConfidence < 0.85; }
    if (Array.isArray(perPage) && perPage.length) doc.ocr.perPage = perPage.map((p) => pick(p, ["page", "confidence"]));
    ...
    doc.ocr.status = "completed"; doc.ocr.textDocId = docText._id; await doc.save();

    if (jobId) {
      const job = await OcrJob.findById(jobId);
      if (job) { job.status = "completed"; job.finishedAt = new Date(); job.metrics = { pages: metrics?.pages, durationMs: metrics?.durationMs, garbageRate: metrics?.garbageRate }; await job.save(); }
    }
...
export const failOcrJob = async (req, res) => {
  try {
    ...
    job.error = { message: req.body?.message || "Unknown OCR error", stack: req.body?.stack };
    if (req.body?.metrics) job.metrics = pick(req.body.metrics, ["pages", "durationMs", "garbageRate"]);
    await job.save();

    await Document.updateOne({ _id: job.documentId, "ocr.status": { $ne: "completed" } }, { $set: { "ocr.status": "failed" } });
...
export const listOcrJobs = async (req, res) => {
  try {
    ...
    if (req.user?.role === "Admin") {
      ...
      if (!caseIds.length) return res.json({ page: pageNum, limit: limitNum, total: 0, items: [] });

      const docs = await Document.find({ orgId: req.orgId, caseId: { $in: caseIds } }).select("_id").lean();
      const docIds = docs.map((d) => d._id);
      if (!docIds.length) return res.json({ page: pageNum, limit: limitNum, total: 0, items: [] });

      filter.documentId = docId ? docId : { $in: docIds };
    }

    const [items, total] = await Promise.all([
      OcrJob.find(filter).sort("-createdAt").skip((pageNum - 1) * limitNum).limit(limitNum).lean(),
